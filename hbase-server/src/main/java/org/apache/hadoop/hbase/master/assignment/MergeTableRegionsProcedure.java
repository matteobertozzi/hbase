...
public class MergeTableRegionsProcedure
  ...
  protected Flow executeFromState(
    ...
    try {
      ...
      case MERGE_TABLE_REGIONS_CLOSE_REGIONS:
        addChildProcedure(createUnassignRegionProcedures(env, getRegionReplication(env)));
      ...
      case MERGE_TABLE_REGIONS_OPEN_MERGED_REGION:
        addChildProcedure(createAssignRegionProcedures(env, getRegionReplication(env)));
  ...
  private void rollbackCloseRegionsForMerge(final MasterProcedureEnv env) throws IOException {
    ...
    final AssignRegionProcedure[] procs =
        new AssignRegionProcedure[regionsToMerge.length * regionReplication];
    int procsIdx = 0;
    for (int i = 0; i < regionsToMerge.length; ++i) {
      for (int j = 0; j < regionReplication; ++j) {
        final HRegionInfo hri = RegionReplicaUtil.getRegionInfoForReplica(regionsToMerge[i], j);
        procs[procsIdx++] = env.getAssignmentManager().createAssignRegionProcedure(hri, serverName);
      }
    }
    env.getMasterServices().getMasterProcedureExecutor().submitProcedures(procs);
  }

  private UnassignRegionProcedure[] createUnassignRegionProcedures(final MasterProcedureEnv env,
      final int regionReplication) {
    final UnassignRegionProcedure[] procs =
        new UnassignRegionProcedure[regionsToMerge.length * regionReplication];
    int procsIdx = 0;
    for (int i = 0; i < regionsToMerge.length; ++i) {
      for (int j = 0; j < regionReplication; ++j) {
        final HRegionInfo hri = RegionReplicaUtil.getRegionInfoForReplica(regionsToMerge[i], j);
        procs[procsIdx++] = env.getAssignmentManager().createUnassignRegionProcedure(hri,null,true);
      }
    }
    return procs;
  }

  private AssignRegionProcedure[] createAssignRegionProcedures(final MasterProcedureEnv env,
      final int regionReplication) {
    final ServerName targetServer = getServerName(env);
    final AssignRegionProcedure[] procs = new AssignRegionProcedure[regionReplication];
    for (int i = 0; i < procs.length; ++i) {
      final HRegionInfo hri = RegionReplicaUtil.getRegionInfoForReplica(mergedRegion, i);
      procs[i] = env.getAssignmentManager().createAssignRegionProcedure(hri, targetServer);
    }
    return procs;
  }
