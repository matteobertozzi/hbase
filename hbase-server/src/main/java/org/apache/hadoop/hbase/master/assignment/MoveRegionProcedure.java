  ...
  protected Flow executeFromState(final MasterProcedureEnv env, final MoveRegionState state)
    ...
    switch (state) {
      case MOVE_REGION_UNASSIGN:
        addChildProcedure(new UnassignRegionProcedure(plan.getRegionInfo(), plan.getDestination(), true));
        setNextState(MoveRegionState.MOVE_REGION_ASSIGN);
        break;
      case MOVE_REGION_ASSIGN:
        addChildProcedure(new AssignRegionProcedure(plan.getRegionInfo(), plan.getDestination()));
