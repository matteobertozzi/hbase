...
public class AssignRegionProcedure extends RegionTransitionProcedure {
  private static final Log LOG = LogFactory.getLog(AssignRegionProcedure.class);
  ...
  public AssignRegionProcedure() {
    // Required by the Procedure framework to create the procedure on replay
  }

  public AssignRegionProcedure(final HRegionInfo regionInfo) {
    this(regionInfo, false);
  }

  public AssignRegionProcedure(final HRegionInfo regionInfo, final boolean forceNewPlan) {
    super(regionInfo);
    this.forceNewPlan = forceNewPlan;
    this.targetServer = null;
  }

  public AssignRegionProcedure(final HRegionInfo regionInfo, final ServerName targetServer) {
    super(regionInfo);
  ...
  protected boolean startTransition(final MasterProcedureEnv env, final RegionStateNode regionNode)
      throws IOException {
    LOG.info("startTransition " + regionNode);
    // if the region is already open we can't do much...
    if (regionNode.isInState(State.OPEN) && isServerOnline(env, regionNode)) {
      LOG.info("the region is already assigned: " + regionNode);
      return false;
    }

    // if the region has SPLIT or offline flag (set on split), the region is not assignable.
    if (regionNode.isInState(State.SPLIT) || regionNode.getRegionInfo().isOffline()) {
      LOG.info("the region is not assignable: " + regionNode);
      return false;
    }
    ...
    if (aborted.get() && regionNode.isInState(State.CLOSED, State.OFFLINE)) {
      if (incrementAndCheckMaxAttempts(env, regionNode)) {
        ...
        setFailure(getClass().getSimpleName(),
          new RetriesExhaustedException("max attemps exceeded"));
      } else {
    ...
    regionNode.setState(State.CLOSED);
    if (forceNewPlan) {
  ...
  protected boolean updateTransition(final MasterProcedureEnv env, final RegionStateNode regionNode)
    ...
    LOG.info("updateTransition " + regionNode);
    ...
    if (!isServerOnline(env, regionNode)) {
      ...
      LOG.info("the server is not online");
      setTransitionState(RegionTransitionState.REGION_TRANSITION_QUEUE);
      return true;
    }
    ...
    // Add the open region operation the the server dispatch queue.
  ...
  protected void reportTransition(final MasterProcedureEnv env, final RegionStateNode regionNode,
    ...
    switch (code) {
      case OPENED:
        LOG.debug("report " + code + " transition " + regionNode + " openSeqNum=" + openSeqNum);
        if (openSeqNum < 0) {
          throw new UnexpectedStateException(String.format(
            "reported unexpected openSeqNum=%s transition state=%s for region=%s server=%s",
            openSeqNum, code, regionNode.getRegionInfo(), regionNode.getRegionLocation()));
        }
        regionNode.setOpenSeqNum(openSeqNum);
        setTransitionState(RegionTransitionState.REGION_TRANSITION_FINISH);
        break;
      case FAILED_OPEN:
        LOG.warn("report " + code + " transition " + regionNode);
        handleFailure(env, regionNode);
        break;
      default:
        throw new UnexpectedStateException(String.format(
          "reported unexpected transition state=%s for region=%s server=%s" +
