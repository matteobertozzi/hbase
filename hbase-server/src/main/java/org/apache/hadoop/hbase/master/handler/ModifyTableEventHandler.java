...
public abstract class ModifyTableEventHandler extends TableEventHandler {
  private static final Log LOG = LogFactory.getLog(ModifyTableEventHandler.class);

  protected boolean persistedToZk = false;

  public ModifyTableEventHandler(EventType eventType, byte[] tableName, Server server,
      MasterServices masterServices) throws IOException {
    super(eventType, tableName, server, masterServices);
    try {
 ...
 private boolean isOnlineSchemaChangeAllowed() {
  ...
  public void process() {
    try {
      ...
      List<HRegionInfo> hris = MetaReader.getTableRegions(this.server.getCatalogTracker(),
        tableName);
  ...
  protected abstract void handleTableOperation(List<HRegionInfo> regions) throws IOException,
      KeeperException;
}
