...
public class MasterProcedureScheduler extends AbstractProcedureScheduler {
  ...
  public static class ServerQueue extends QueueImpl<ServerName> {
    ...
    public boolean requireExclusiveLock(Procedure proc) {
      final ServerProcedureInterface spi = (ServerProcedureInterface)proc;
   ...
   * Based on the table operation type the xlock or the shared-lock will be taken.
   * @param procedure the procedure trying to acquire the lock
   * @return true if the procedure has to wait for the table to be available
   */
  public boolean waitTableLock(final Procedure procedure) {
    TableProcedureInterface tpi = (TableProcedureInterface)procedure;
    if (requireTableExclusiveLock(tpi)) {
      return waitTableExclusiveLock(procedure, tpi.getTableName());
    }
    return waitTableSharedLock(procedure, tpi.getTableName());
  }
   ...
   */
  public void wakeTableLock(final Procedure procedure) {
    TableProcedureInterface tpi = (TableProcedureInterface)procedure;
    if (requireTableExclusiveLock(tpi)) {
      wakeTableExclusiveLock(procedure, tpi.getTableName());
    } else {
      wakeTableSharedLock(procedure, tpi.getTableName());
    }
  }
  ...
  public boolean waitTableExclusiveLock(final Procedure procedure, final TableName table) {
    ...
    try {
      ...
      if (!nsQueue.trySharedLock(procedure)) {
  ...
  private TableQueue waitTableQueueSharedLock(final Procedure procedure, final TableName table) {
    ...
    try {
      ...
      if (!tableQueue.trySharedLock(procedure)) {
   ...
   * @see #wakeExclusiveLock(Procedure,ServerName)
   * @param procedure the procedure trying to acquire the lock
  ...
  private static abstract class Queue<TKey extends Comparable<TKey>>
    ...
    public boolean trySharedLock(final Procedure proc) {
