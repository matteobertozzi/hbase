...
class SnapshotLogSplitter {
  ...
  private static final String SNAPSHOT_REGION_MARKER = "_$SNAPSHOT_NOT_A_REGION$_";
    ...
    try {
      ...
      while ((entry = log.next()) != null) {
        ...
        if (!Bytes.equals(regionName, key.getEncodedRegionName())) {
          // Stop on region marker
          if (Bytes.toString(key.getEncodedRegionName()).startsWith(SNAPSHOT_REGION_MARKER)) {
            break;
          }
