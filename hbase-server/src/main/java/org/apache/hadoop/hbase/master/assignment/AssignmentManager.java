...
public class AssignmentManager implements ServerListener {
  ...
  //  - handle sys table split/assignment first (e.g. acl, namespace)
  ...
  public void assignMeta(final HRegionInfo metaRegionInfo, final ServerName serverName)
      throws IOException {
    assert isMetaRegion(metaRegionInfo) : "unexpected non-meta region " + metaRegionInfo;
    AssignRegionProcedure proc;
    if (serverName != null) {
      LOG.debug("try assigning Meta " + metaRegionInfo + " to " + serverName);
      proc = createAssignRegionProcedure(metaRegionInfo, serverName);
    } else {
      LOG.debug("assigning Meta " + metaRegionInfo);
      proc = createAssignRegionProcedure(metaRegionInfo, false);
    }
  ...
  public void assign(final HRegionInfo regionInfo, final boolean forceNewPlan) throws IOException {
    AssignRegionProcedure proc = createAssignRegionProcedure(regionInfo, forceNewPlan);
    ProcedureSyncWait.submitAndWaitProcedure(master.getMasterProcedureExecutor(), proc);
  }
  ...
  // ============================================================================================
  public AssignRegionProcedure[] createAssignRegionProcedures(final Collection<HRegionInfo> regionInfo) {
    return createAssignRegionProcedures(regionInfo, false);
  }

  public AssignRegionProcedure[] createAssignRegionProcedures(final Collection<HRegionInfo> regionInfo,
      final boolean forceNewPlan) {
    final AssignRegionProcedure[] procs = new AssignRegionProcedure[regionInfo.size()];
    int index = 0;
    for (HRegionInfo hri: regionInfo) {
      procs[index++] = createAssignRegionProcedure(hri, forceNewPlan);
    }
    return procs;
  }

  public UnassignRegionProcedure[] createUnassignRegionProcedures(final Collection<HRegionInfo> regionInfo) {
    final UnassignRegionProcedure[] procs = new UnassignRegionProcedure[regionInfo.size()];
    int index = 0;
    for (HRegionInfo hri: regionInfo) {
      procs[index++] = createUnassignRegionProcedure(hri, null, false);
    }
    return procs;
  }
  ...
  /**
   * Called by things like EnableTableProcedure to get a list of AssignRegionProcedure
   * to assign the regions of the table.
   */
  public AssignRegionProcedure[] createAssignRegionProcedures(final TableName tableName) {
    return createAssignRegionProcedures(regionStateMap.getRegionsOfTable(tableName));
  }

  /**
   * Called by things like DisableTableProcedure to get a list of UnassignRegionProcedure
   * to unassign the regions of the table.
   */
  public UnassignRegionProcedure[] createUnassignRegionProcedures(final TableName tableName) {
    return createUnassignRegionProcedures(regionStateMap.getRegionsOfTable(tableName));
  }
  ...
  public AssignRegionProcedure createAssignRegionProcedure(final HRegionInfo regionInfo,
      final boolean forceNewPlan) {
    AssignRegionProcedure proc = new AssignRegionProcedure(regionInfo, forceNewPlan);
  ...
  public AssignRegionProcedure createAssignRegionProcedure(final HRegionInfo regionInfo,
    ...
    AssignRegionProcedure proc = new AssignRegionProcedure(regionInfo, targetServer);
    proc.setOwner(getProcedureEnvironment().getRequestUser().getShortName());
    return proc;
  }

  public UnassignRegionProcedure createUnassignRegionProcedure(final HRegionInfo regionInfo,
      final ServerName destinationServer, final boolean force) {
    UnassignRegionProcedure proc = new UnassignRegionProcedure(regionInfo, destinationServer, force);
    proc.setOwner(getProcedureEnvironment().getRequestUser().getShortName());
    return proc;
  }
  ...
  public void checkOnlineRegionsReportForMeta(final ServerStateNode serverNode,
    ...
    try {
      ...
      final ServerName serverName = serverNode.getServerName();
      LOG.error("Killing server=" + serverName + ": " + e.getMessage());
  ...
  private boolean processofflineServersWithOnlineRegions() {
    ...
    for (RegionStateNode regionNode: regionStateMap.getRegionNodes()) {
      if (regionNode.isInState(State.OPEN)) {
        ...
        if (!master.getServerManager().isServerOnline(serverName)) {
          LOG.debug("offline server " + serverName +
            " has OPEN region " + regionNode.getRegionInfo());
    ...
    for (HRegionInfo regionInfo: regionsToAssign) {
      master.getMasterProcedureExecutor().submitProcedure(
        createAssignRegionProcedure(regionInfo, false));
    }
    et = System.currentTimeMillis();
