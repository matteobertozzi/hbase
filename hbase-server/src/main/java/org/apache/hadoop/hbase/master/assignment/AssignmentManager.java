 ...
 *
 * Regions are created by CreateTableProcedure, Split?
 * Regions are deleted by DeleteTableProcedure, Merge?
 * TODO...
 */
...
public class AssignmentManager implements ServerListener {
  ...
  // Used by TestSplitTransactionOnClusterf
  ...
  public void deleteTable(final TableName tableName) {
    final ArrayList<RegionStateNode> regions = regionStateMap.getTableRegionStateNodes(tableName);
    for (int i = 0; i < regions.size(); ++i) {
      final HRegionInfo regionInfo = regions.get(i).getRegionInfo();
  ...
  public ReportRegionStateTransitionResponse reportRegionStateTransition(
    ...
    } catch (UnexpectedStateException|UnsupportedOperationException e) {
  ...
  private void updateRegionSplitTransition(final ServerName serverName, final TransitionCode state,
    ...
    // TODO: Attach split support
    throw new UnsupportedOperationException(String.format(
      "Split not handled yet: state=%s parent=%s hriA=%s hriB=%s", state, parent, hriA, hriB));
  }
  ...
  private static class RegionInTransitionChore extends ProcedureInMemoryChore<MasterProcedureEnv> {
    ...
    protected void periodicExecute(final MasterProcedureEnv env) {
      ...
      final int ritThreshold = env.getMasterConfiguration().
      ...
      for (RegionState state: am.getRegionStates().getRegionsStateInTransition()) {
        totalRITs++;
        final long ritTime = currentTime - state.getStamp();
        if (ritTime > ritThreshold) {
          am.handleRegionOverStuckWarningThreshold(state.getRegion());
      ...
      // update metrics
      am.updateRegionsInTransitionMetrics(totalRITs, totalRITsOverThreshold, oldestRITTime);
    }
  }

  private void updateRegionsInTransitionMetrics(final int totalRITs,
      final int totalRITsOverThreshold, final long oldestRITTime) {
    metrics.updateRITOldestAge(oldestRITTime);
    metrics.updateRITCount(totalRITs);
    metrics.updateRITCountOverThreshold(totalRITsOverThreshold);
  }
  ...
  private void loadMeta(final Result result) throws IOException {
    ...
    for (int i = 0; i < locations.length; ++i) {
      ...
      final State state = regionStateStore.getRegionState(result, replicaId);

      final ServerName lastHost = hrl.getServerName();
      final ServerName regionLocation = regionStateStore.getRegionServer(result, replicaId);
