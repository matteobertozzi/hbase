...
public class StoreFile {
  ...
  /** Regex that will work for plain hfiles and hfile links */
  private static final Pattern HFILE_NAME_PARSER =
    Pattern.compile("((?:[a-zA-Z_0-9][a-zA-Z_0-9\\.\\-\\@]*)[0-9a-f]+(?:_SeqId_[0-9]+_)?)");
  ...
  private static boolean isHFile(final Path path) {
    Matcher m = HFILE_NAME_PARSER.matcher(path.getName());
  ...
  public static boolean isReference(final String name) {
    Matcher m = REF_NAME_PARSER.matcher(name);
