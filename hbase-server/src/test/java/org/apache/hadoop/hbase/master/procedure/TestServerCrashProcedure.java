...
public class TestServerCrashProcedure {
  ...
  /**
   * Run server crash procedure steps twice to test idempotency
   * and that we are persisting all needed state.
   */
  private void testRecoveryAndDoubleExecution(final boolean carryingMeta) throws Exception {
    ...
    try {
      ...
      final ServerName rsToKill = AssignmentTestingUtil.getServer(util, tableName, carryingMeta);
      ...
      master.getServerManager().moveFromOnelineToDeadServers(rsToKill);
