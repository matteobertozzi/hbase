...
import org.apache.hadoop.hbase.master.assignment.RegionAlreadyAssignedException;
import org.apache.hadoop.hbase.master.assignment.RegionNotAssignedException;
...
public class TestAssignmentManager {
  ...
  public void testAssignWithRandExec() throws Exception {
    ...
    AssignRegionProcedure proc = am.createAssignRegionProcedure(hri, false);
    //waitOnFuture(submitProcedure(am.createAssignRegionProcedure(hri, false, false)));
    // TODO
  }

  @Test(timeout=60000)
  public void testSocketTimeout() throws Exception {
    final TableName tableName = TableName.valueOf("testSocketTimeout");
    final HRegionInfo hri = createRegionInfo(tableName, 1);

    rsDispatcher.setMockRsExecutor(new SocketTimeoutRsExecutor(20, 3));
    waitOnFuture(submitProcedure(am.createAssignRegionProcedure(hri, false)));

    rsDispatcher.setMockRsExecutor(new SocketTimeoutRsExecutor(20, 3));
    waitOnFuture(submitProcedure(am.createUnassignRegionProcedure(hri, null, false)));
  }
  ...
  private void testAssign(final MockRSExecutor executor, final int nregions) throws Exception {
    ...
    AssignRegionProcedure[] assignments = new AssignRegionProcedure[nregions];
  ...
  public void testAssignAnAssignedRegion() throws Exception {
    ...
    final Future<byte[]> futureA = submitProcedure(am.createAssignRegionProcedure(hri, false));
    final Future<byte[]> futureB = submitProcedure(am.createAssignRegionProcedure(hri, false));
    ...
    try {
      ...
      fail("expected RegionAlreadyAssignedException");
    } catch (RegionAlreadyAssignedException e) {
  ...
  public void testUnassignAnUnassignedRegion() throws Exception {
    ...
    final Future<byte[]> futureA = submitProcedure(am.createUnassignRegionProcedure(hri, null, false));
    final Future<byte[]> futureB = submitProcedure(am.createUnassignRegionProcedure(hri, null, false));

    // wait first unassign
    waitOnFuture(futureA);
    am.getRegionStates().isRegionInState(hri, State.CLOSED);

    // wait second unassign
    try {
      waitOnFuture(futureB);
      fail("expected RegionNotAssignedException");
    } catch (RegionNotAssignedException e) {
      LOG.info("got expected exception: " + e.getMessage());
    }
  ...
  //  Helpers
  // ============================================================================================
  private void bulkSubmit(final AssignRegionProcedure[] procs) throws Exception {
  ...
  private AssignRegionProcedure createAndSubmitAssign(TableName tableName, int regionId) {
  ...
  private UnassignRegionProcedure createAndSubmitUnassign(TableName tableName, int regionId) {
    HRegionInfo hri = createRegionInfo(tableName, regionId);
    UnassignRegionProcedure proc = am.createUnassignRegionProcedure(hri, null, false);
    master.getMasterProcedureExecutor().submitProcedure(proc);
    return proc;
  }
  ...
  private class SocketTimeoutRsExecutor extends GoodRsExecutor {
    ...
    public ExecuteProceduresResponse sendRequest(ServerName server, ExecuteProceduresRequest req)
      ...
      if (sockTimeoutRetries++ < maxSocketTimeoutRetries) {
        ...
        master.getServerManager().moveFromOnelineToDeadServers(server);
