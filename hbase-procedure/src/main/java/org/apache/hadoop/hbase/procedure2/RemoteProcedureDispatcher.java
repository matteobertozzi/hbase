...
public abstract class RemoteProcedureDispatcher<TEnv, TRemote extends Comparable<TRemote>> {
  ...
  public boolean start() {
    ...
    LOG.info("Starting procedure remote dispatcher - threads=" + corePoolSize);
  ...
  public boolean stop() {
    ...
    LOG.info("Stopping the procedure remote dispatcher");
  ...
  public void join() {
    ...
    try {
      ...
      LOG.warn("got interrupted while waiting for thread-pool termination", e);
    }
  }
  ...
  protected final class BufferNode extends DelayedContainerWithTimestamp<TRemote>
    ...
    public synchronized void add(final RemoteProcedure operation) {
      if (this.operations == null) {
        ...
        setTimeoutTimestamp(EnvironmentEdgeManager.currentTime() + operationDelay);
