...
public class ProcedureStoreTracker {
  ...
  public void setDeletedIfSet(final ProcedureStoreTracker tracker) {
    ...
    for (BitSetNode node: map.values()) {
      ...
      for (long procId = minProcId; procId <= maxProcId; ++procId) {
        ...
        if (isUpdatedOrDeleted(trackerNode, procId)) {
          node.delete(procId);
        }
      }
    }
  ...
  private static boolean isUpdatedOrDeleted(final BitSetNode node, final long procId) {
    if (node == null || !node.contains(procId)) return false;
    return node.isUpdated(procId) || node.isDeleted(procId) == DeleteState.YES;
  }
